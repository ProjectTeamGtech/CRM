...
public int saveClient(Client client, int id) {
	PreparedStatement preparedStatement = null;
	int result = 0;
	String sql = "insert into apm_patient(firstname,mobno,email,town,country,client_type,lastModified)"+ "values(?,?,?,?,?,?,?)";
	...
	try{
		...
		preparedStatement.setString(7, client.getLastModifiedDate());
		result = preparedStatement.executeUpdate();
		if(result == 1){
			ResultSet resultSet = preparedStatement.getGeneratedKeys();
			if(resultSet.next()){
